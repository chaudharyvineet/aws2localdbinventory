...
# Define fallback configurations for spot instances
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if event['resource'] == '/provision-machine':
            ...
            # For spot instances, get the spot request ID
            ...
            if request_id:  # Spot instance
                ...
                if spot_request['State'] == 'failed':
                    ...
                    current_instance = ec2_client.describe_instances(InstanceIds=[instance_id])['Reservations'][0]['Instances'][0]
                    current_type = current_instance['InstanceType']
                    current_az = current_instance['Placement']['AvailabilityZone']
                    
                    availability_zones = get_availability_zones()
                    fallback_types = INSTANCE_TYPE_FALLBACKS.get(current_type, [])
                    
                    # Try all configurations
                    for inst_type in [current_type] + fallback_types:
                        for az in availability_zones:
                            ...
                            try:
                                ...
                                new_response = ec2_client.run_instances(**instance_params)
                                new_instance_id = new_response['Instances'][0]['InstanceId']
                                
                                # Get new spot request ID
                                spot_requests = ec2_client.describe_spot_instance_requests(
                                    Filters=[{'Name': 'instance-id', 'Values': [new_instance_id]}]
                                )
                                new_request_id = spot_requests['SpotInstanceRequests'][0]['SpotInstanceRequestId']
                    ...
                    return {
                        'statusCode': 400,
                        'body': json.dumps({
                            'error': 'No spot capacity available in any configuration'
                        })
                    }
